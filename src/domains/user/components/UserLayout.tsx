...
const UserLayout: React.FC<UserLayoutProps> = ({ children }) => {
    ...
    const [collapsedSections, setCollapsedSections] = useState<Set<number>>(new Set()); // All sections open by default for users

    const navigationSections = getNavigationForRole(user?.role || 'user');
    ...
    const updateScrollIndicators = (scrollContainer: HTMLElement | null) => {
        ...
        const topIndicator = document.getElementById('scroll-top-indicator-user');
        const bottomIndicator = document.getElementById('scroll-bottom-indicator-user');
        ...
        if (topIndicator && bottomIndicator) {
            ...
            console.log('UserLayout Scroll:', { scrollTop, scrollHeight, clientHeight, hasContentAbove, hasContentBelow });
        }
    };

    React.useEffect(() => {
        const scrollContainer = document.querySelector('.user-sidebar-scroll') as HTMLElement;
        if (scrollContainer) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-background via-surface to-background flex">
            ...
            <aside className={`bg-surface-elevated border-r border-border flex flex-col max-h-screen transition-all duration-300 ${sidebarCollapsed ? 'w-16' : 'w-80'
                    ...
                    <div className="flex items-center justify-between">
                        {!sidebarCollapsed && (
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-lg font-semibold text-foreground font-raleway">ShareGarauna</h2>
                                    <p className="text-xs text-foreground-muted">User Dashboard</p>
                                </div>
                            </div>
                        )}
                        <Button
                ...
                <div className="flex-1 overflow-y-auto relative user-sidebar-scroll">
                                                ...
                                                return (
                                                    <li key={item.name}>
                                                        {isExternal ? (
                                                            ...
                                                            >
                                                                <div className={`flex-shrink-0 ${isActive(item.href) ? 'text-primary-foreground' : 'text-foreground-muted group-hover:text-foreground'}`}>
                                                                    {getIcon(item.icon)}
                                                                </div>
                                                                {!sidebarCollapsed && (
                                                                    <div className="flex items-center gap-2 flex-1 min-w-0">
                                                                        <span className="truncate">{item.name}</span>
                                                                        <ExternalLink className={`h-3 w-3 flex-shrink-0 ${isActive(item.href) ? 'text-primary-foreground' : 'text-foreground-muted group-hover:text-foreground'}`} />
                                                                    </div>
                                                                )}
                                                            </a>
                                                        ) : (
